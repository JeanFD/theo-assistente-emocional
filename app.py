...
class App:
    def __init__(self):
        ...
        # --- TTS e Fala ---
        self.tts = TTS(voice_folder=self.config.get('voz', 'masculino'))
        self.falando = False
        ...
        if cfg_path.exists():
            try:
                with open(cfg_path, 'r', encoding='utf-8') as f:
                    ...
                    self.config['voz'] = loaded_config.get('voz', 'masculino')
    ...
    def _criar_botoes_config(self):
        voz_atual = self.config.get('voz', 'masculino').capitalize()
