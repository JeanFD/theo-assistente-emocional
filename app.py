...
try:
    ...
    # ALTERADO: Importa as funções específicas de envio de dados
    from comunicacao.envio_dados import enviar_dados_perfil, enviar_dados_sentimento, enviar_dados_bpm
except ImportError as e:
    print(f"Erro de importação: {e}")
    print("Verifique se a estrutura de pastas e os nomes dos arquivos (incluindo comunicacao/envio_dados.py) estão corretos.")
    sys.exit()
...
class App:
    def __init__(self):
        ...
        self.registro = {"sentimento": None, "tipo": None, "escala": None} # BPM não é mais armazenado aqui
            ...
            if evento.type == pygame.MOUSEBUTTONDOWN:
                if self.falando:  
                    ...
                    if self.estado == Estado.RESPIRACAO and not self.respiracao_intro_completa:
                        ...
                        if fase_inicial_config.get("audio_key"): # Verifica se há audio_key
                            self.tts.speak(key=fase_inicial_config["audio_key"])
                            self.falando = True
                            self.ultimo_texto_falado_key = fase_inicial_config["audio_key"]
                        # Botões ainda não aparecem aqui, só depois do 1º comando de fase
                    elif self.estado == Estado.RESPIRACAO and self.respiracao_intro_completa and not self.botoes_resp_animacao_iniciada:
                    ...
                    elif self.btn_group and self.btn_group.buttons and self.estado not in [Estado.OBRIGADO, Estado.ALERTA_TRISTEZA, Estado.RESPIRACAO]:
                        self.btn_group.start_ms = pygame.time.get_ticks()
    ...
    def on_click(self, clicked):
        ...
        if self.estado == Estado.CONFIG:
            dados_perfil_para_envio = {}
            voz_mudou = False
            if clicked == 0: # Botão Voz
                self.config['voz'] = 'feminino' if self.config.get('voz', 'robo') == 'masculino' else 'robo'
                self.tts = TTS(voice_folder=self.config['voz'])
                self._criar_botoes_config() 
                dados_perfil_para_envio['voz'] = self.config['voz']
                voz_mudou = True
            elif clicked == 1: 
                if self.config.get('sexo') != "Masculino": self.config['sexo'] = "Masculino"; dados_perfil_para_envio['sexo'] = "Masculino"
            elif clicked == 2: 
                if self.config.get('sexo') != "Feminino": self.config['sexo'] = "Feminino"; dados_perfil_para_envio['sexo'] = "Feminino"
            elif clicked == 3: self.config['idade'] = min(120, self.config.get("idade", 0) + 1); dados_perfil_para_envio['idade'] = self.config['idade']
            elif clicked == 4: self.config['idade'] = max(0, self.config.get("idade", 0) - 1); dados_perfil_para_envio['idade'] = self.config['idade']
            
            if dados_perfil_para_envio : self.config_precisa_atualizar = True

            if clicked == 5: # Botão "Concluir"
                with open("config.json", "w", encoding='utf-8') as f: json.dump(self.config, f, ensure_ascii=False, indent=4)
                enviar_dados_perfil(self.config) # Envia todos os dados do perfil ao concluir
                self._resetar_para_inicio()
            else:
                if dados_perfil_para_envio and not voz_mudou: # Só envia se não foi só mudança de voz (que já recriou botões)
                    enviar_dados_perfil(dados_perfil_para_envio)
                self.indice_selecionado = clicked
                return
                 
        elif self.estado == Estado.INICIO:
            self.registro = {"sentimento": None, "tipo": None, "escala": None}
            ...
            dados_sentimento_payload = {
                "sentimento": self.registro.get("sentimento"),
                "tipo": self.registro.get("tipo"),
                "escala": self.registro.get("escala")
            }
            enviar_dados_sentimento(dados_sentimento_payload)
        ...
        if estado_anterior != self.estado:
            if self.estado not in [Estado.ALERTA_TRISTEZA, Estado.GROUNDING, Estado.CONFIG]: 
                ...
                self.btn_group = GrupoBotoes(self.largura, self.altura, labels_alerta, self.fonte_botao)
            elif self.estado == Estado.CONFIG and estado_anterior != Estado.CONFIG:
                self._criar_botoes_config()
    ...
    def update_tempo(self, dt):
        if self.estado == Estado.BATIMENTO_MEDINDO:
            try:
                ...
                enviar_dados_bpm({"bpm": bpm}) # Envia BPM
    ...
    def render(self):
        ...
        if self.estado != Estado.DORMINDO:
            ...
            if self.estado == Estado.GROUNDING:
                ...
                if self.config_precisa_atualizar:
                    voz_cfg_txt = self.config.get('voz', 'robo').capitalize()
        ...
        if self.falando_primeiro:
            ...
            if self.falando and not self.tts.speaking: # "Fala principal" (intro) ACABOU
                fala_que_acabou = self.ultimo_texto_falado_key
                self.falando = False
                self.falando_primeiro = False 

                if self.estado == Estado.RESPIRACAO and fala_que_acabou == AUDIO_KEYS.get(Estado.RESPIRACAO): # Intro da respiração acabou
                    self.respiracao_intro_completa = True
                    self.respiracao_timer = 0 
                    fase_inicial_config = self.respiracao_config[self.respiracao_fase]
                    if fase_inicial_config.get("audio_key"):
                        self.tts.speak(key=fase_inicial_config["audio_key"])
                        self.falando = True 
                        self.ultimo_texto_falado_key = fase_inicial_config["audio_key"]
                    # Botões de respiração ainda não aparecem aqui, só depois do 1º comando de fase
                elif self.estado == Estado.ALERTA_TRISTEZA and fala_que_acabou == AUDIO_KEYS.get(Estado.ALERTA_TRISTEZA): # Primeira frase do alerta acabou
                    if self.btn_group and self.btn_group.buttons: # Mostra botão "Próximo"
                        self.btn_group.start_ms = pygame.time.get_ticks()
                elif self.btn_group and self.btn_group.buttons: # Para outros estados
                    self.btn_group.start_ms = pygame.time.get_ticks()
                ...
                if self.btn_group and self.btn_group.buttons:
                    self.btn_group.start_ms = pygame.time.get_ticks()
                if self.estado == Estado.RESPIRACAO and not self.respiracao_intro_completa: # Se não tinha intro, mas precisa do 1o comando
                     self.respiracao_intro_completa = True 
                     self.respiracao_timer = 0
                     fase_inicial_config = self.respiracao_config[self.respiracao_fase]
                     if fase_inicial_config.get("audio_key"):
            ...
            if self.falando and not self.tts.speaking: # Fala específica (não principal) terminou
                self.falando = False
                
                if self.estado == Estado.RESPIRACAO and self.respiracao_intro_completa and not self.botoes_resp_animacao_iniciada:
                    # Se o primeiro comando de fase ("Inspire") acabou, OU se não houve intro E não houve primeiro comando
                    if self.ultimo_texto_falado_key == self.respiracao_config[FaseRespiracao.INSPIRAR]["audio_key"] or \
                       (not AUDIO_KEYS.get(Estado.RESPIRACAO) and not self.precisa_falar_primeiro_comando_respiracao):
                        if self.btn_group and self.btn_group.buttons:
                            self.btn_group.start_ms = pygame.time.get_ticks()
                        self.botoes_resp_animacao_iniciada = True
                
                elif self.estado == Estado.ALERTA_TRISTEZA and self.ultimo_texto_falado_key in self.alerta_audio_keys_frases:
                    if self.btn_group and self.btn_group.buttons: 
                        self.btn_group.start_ms = pygame.time.get_ticks()
                
                elif self.btn_group and self.btn_group.buttons and \
                     self.estado not in [Estado.RESPIRACAO, Estado.GROUNDING, Estado.OBRIGADO, Estado.ALERTA_TRISTEZA]:
                     self.btn_group.start_ms = pygame.time.get_ticks()

# --- Ponto de Entrada da Aplicação (se este arquivo for executado diretamente) ---
