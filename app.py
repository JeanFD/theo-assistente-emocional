...
STATE_CONFIG = {
    ...
    Estado.BATIMENTO_RESULTADO: ("", []), # Texto dinâmico, sem botões fixos. Interação via clique/tecla na tela.
...
class App:
                    ...
                    if self.estado == Estado.OBRIGADO:
                        ...
                        if self.alerta_frase_atual_idx < len(self.alerta_frases):
                            ...
                            if self.alerta_frase_atual_idx == len(self.alerta_frases) - 1:
                                ...
                                self.indice_selecionado = 0
                            elif self.btn_group and self.btn_group.buttons: # Garante que o botão "Próximo" apareça
                                self.btn_group.start_ms = pygame.time.get_ticks()
                        else:
                            self._resetar_para_inicio()
                    return 
    ...
    def on_click(self, clicked):
        ...
        if self.estado == Estado.CONFIG:
            ...
            if clicked == -1: # Sinal de handle_events para avançar após clique/tecla
                self._resetar_para_inicio()
            ...
            if clicked == -1: # Sinal de handle_events para avançar
                self._resetar_para_inicio()
        ...
        if estado_anterior != self.estado:
            if self.estado not in [Estado.ALERTA_TRISTEZA, Estado.GROUNDING]: 
    ...
    def render(self):
        ...
        if self.estado != Estado.DORMINDO:
            ...
            if self.estado == Estado.GROUNDING:
                ...
                # CORREÇÃO AQUI: Desenha os botões se a animação foi iniciada, independente de estar falando os comandos de fase
            ...
            if self.falando and not self.tts.speaking: # "Fala principal" (intro) ACABOU
                ...
                if self.estado == Estado.RESPIRACAO and self.precisa_falar_primeiro_comando_respiracao:
                    ...
                    if fase_audio_key:
                        ...
                        self.ultimo_texto_falado_key = fase_audio_key
                    self.precisa_falar_primeiro_comando_respiracao = False
                    # Não seta falando_primeiro = False aqui, pois uma sub-fala (1o comando) foi iniciada
                    # Não inicia animação dos botões de respiração ainda.
                elif self.estado == Estado.ALERTA_TRISTEZA:
                    self.falando_primeiro = False # Primeira frase do alerta acabou
                    if self.btn_group and self.btn_group.buttons: # Mostra botão "Próximo"
                        self.btn_group.start_ms = pygame.time.get_ticks()
                else: # Para todos os outros estados, ou se respiração não precisava do 1o comando
                    self.falando_primeiro = False 
                    if self.btn_group and self.btn_group.buttons:
                        self.btn_group.start_ms = pygame.time.get_ticks()
                    if self.estado == Estado.RESPIRACAO: # Se intro da respiração acabou E não precisava do 1o comando
                        self.botoes_resp_animacao_iniciada = True # Botões podem aparecer agora

            elif not chave_audio_principal_estado and self.estado != Estado.BATIMENTO_RESULTADO: 
                self.falando_primeiro = False
                if self.btn_group and self.btn_group.buttons:
                    self.btn_group.start_ms = pygame.time.get_ticks()
                if self.estado == Estado.RESPIRACAO and not self.precisa_falar_primeiro_comando_respiracao:
                     self.botoes_resp_animacao_iniciada = True
        else: # Não é self.falando_primeiro 
            if self.falando and not self.tts.speaking: # Fala específica (não principal) terminou
                self.falando = False
                
                if self.estado == Estado.RESPIRACAO and \
                   not self.botoes_resp_animacao_iniciada and \
                   (self.ultimo_texto_falado_key == self.respiracao_config[FaseRespiracao.INSPIRAR]["audio_key"] or not AUDIO_KEYS.get(Estado.RESPIRACAO)): # Se o 1o comando acabou ou não houve intro
                    if self.btn_group and self.btn_group.buttons:
                        self.btn_group.start_ms = pygame.time.get_ticks()
                    self.botoes_resp_animacao_iniciada = True
